...
<div class="w-full lg:w-2/3 xl:w-2/3 2xl:w-2/3 mx-3 my-3 mx-auto rounded-3xl p-3 bg-white">
 <h2 class="capitalize text-center text-tiruhakim text-3xl border-b-2 border-gray-300 p-2">submit the form below to register Doctor</h2>
 <form @submit.prevent="claimProfile" enctype="multipart/form-data">
  ...
  <input type="email" v-model="email" placeholder="email" required class="text-tiruhakim m-1 border-2 border-gray-200 rounded-xl w-full py-1.5 px-2 text-xl outline-none focus:border-b-2"/>
  <p v-if="emailError" class="text-xl text-red-600">Email taken! a Doctor with this email is already registered</p>
  <label for="speciality" class="capitalize m-1 text-2xl text-tiruhakim m-1">speciality</label>
  <div class="flex">
    <select v-model="selectedSpeciality" @change="selectSpeciality" class="border-2 border-tiruhakim text-tiruhakim text-xl rounded-md px-3 py-1 ml-1 mr-3 my-1">
      ...
      <div>
       ...
       <div v-if="popup" class="absolute top-1/4 left-1/3 bg-white z-20 rounded-xl py-5 px-10">
        ...
        <input @keyup.enter="addSpeciality" type="text" v-model="specialityName" required placeholder="new speciality" :class="[error? 'border-red-500':'border-gray-200']" class="m-1 border-2 rounded-xl w-72 p-2 text-xl outline-none focus:border-b-2 focus:border-b-tiruhakim">
  ...
  <div class="my-3">
    ...
    <input type="file" @change="onChange" required accept="image/*" class="rounded-lg m-1 text-xl text-tiruhakim border-2 border-tiruhakim"/>
  </div>
    ...
    <div v-if="filteredHospitals&&hospital">
       <div v-for="hospital in filteredHospitals" :key="hospital.id" class="bg-white hover:bg-gray-100 border-b-2 border-gray-300 rounded-lg">
        ...
        <div class="my-auto">
          <button @click="addHospital(hospital.id)" class="bg-tiruhakim text-xl text-white py-2 px-4 rounded-xl">add</button>
        </div>
       </div>
      </div>
  </div>
  <label class="capitalize text-2xl text-tiruhakim m-1 my-2">experience(in years)</label>
   ...
   <input type="number" v-model="age" max="60" min="25" placeholder="age" class="m-1 text-xl p-2 w-32 rounded-xl w-20 border-2 border-gray-200">
   <div class="flex">
   ...
   <input v-else type="submit" :disabled="emailError" value="Add Doctor" class="my-3 py-2 px-5 rounded-3xl text-xl bg-tiruhakim text-white p-1">
 </form>
</div>    
</template>
<script>
...
export default {
  ...
  methods:{
    ...
    selectSpeciality(){
      this.specialities.push(this.selectedSpeciality);
    },
    addSpeciality(){
    ...
    addHospital(id){
      this.hospitalId.push(id);
      this.allHospitals.forEach(hospital=>{
       if(hospital.id==id){
         this.addedHospitals.push(hospital);
       }
      });
      this.hospital='';
    ...
    claimProfile(){
      ...
      axios.post('/associate/adddoctor',formData,config)
      .then(res=>{
        this.$toaster.success('doctor reistered successfully',{timeout:null});
