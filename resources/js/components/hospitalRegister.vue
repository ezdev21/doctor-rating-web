 ...
 <div class="bg-white w-full lg:w-2/3 xl:w-2/3 2xl:w-2/3 rounded-2xl p-2 my-2 mx-auto">
  ...
  <form method="POST" @submit.prevent="register" enctype="multipart/form-data" class="mx-3 my-2">
   ...
   <div class="my-2 flex">
    ...
    <button v-if="city=='select'" @click="popup=true;" class="bg-tiruhakim text-white py-2 px-4 rounded-xl text-xl">add city</button>
   </div>
   <div v-if="popup" class="absolute inset-center bg-white z-20 rounded-xl py-5 px-10">
      ...
      <input @keyup.enter="addCity" type="text" v-model="cityName" required placeholder="city name" class="m-1 border-2 rounded-xl w-72 p-2 text-xl outline-none focus:border-b-2 focus:border-b-tiruhakim">
      <button @click="addCity" class="m-1 flex bg-tiruhakim text-white text-xl py-2 px-4 rounded-2xl">
    ...
    <input @change="onChange" type="file" accept="image/*" required class="text-xl bg-tiruhakim text-white"/>
   </div>
   <div>
   ...
   <input type="submit" :disabled="emailError" :value="registerText" :class="[processing? 'cursor-wait':'',emailError? 'cursor-not-allowed' : '']" class="my-3 py-2 px-5 rounded-3xl text-xl bg-tiruhakim text-white p-1">
  </form> 
...
export default {
 ...
 methods:{
  ...
  addCity(){
    axios.post('/city/add',{cityName:this.cityName})
    .then(res=>{
     this.popup=false;
  ...
  register(){
    this.registerText='processing';
      ...
      axios.post('/hospital/register',formData,config)
      .then(res=>{
